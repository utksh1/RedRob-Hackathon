...
Reference for the 23 behavioral signals in each candidate's redrob_signals object. Includes documentation of the trap candidates in the dataset.
...
gunzip -k candidates.jsonl.gz   # -k keeps the .gz; you get both fileswc -l candidates.jsonl          # should print 100000
Or load the gzipped file directly in Python:
import gzip, jsonwith gzip.open("candidates.jsonl.gz", "rt") as f:    candidates = [json.loads(line) for line in f if line.strip()]print(len(candidates))  # 100000
3. Build your ranker
